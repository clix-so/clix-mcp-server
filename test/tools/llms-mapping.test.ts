...
describe("LLMS Mapping (local file)", () => {
  ...
  it("live: each per-SDK llms.txt URL in mapping should be accessible (HTTP 200)", async () => {
  ...
  it("live: each entry link inside per-SDK llms.txt should be accessible (HTTP 200)", async () => {
    const originalFetch: any = (global as any).fetch;
    if (typeof originalFetch !== "function") {
      return;
    }
